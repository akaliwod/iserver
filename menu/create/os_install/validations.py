...
def validate_server(ctx, iaccount, name_filter, ip_filter, serial_filter, workflow=86400):
    server = common.get_selected_server(ctx, iaccount, name_filter, ip_filter, serial_filter, details=False, workflow=workflow)
    if server is None:
        return None
...
def get_static_attributes(ctx, iaccount, name_filter, ip_filter, serial_filter, scu_name, image_name, interface_name, interface_mac, address, prefix, gateway, nameserver, hostname, password, organization_name):
    if name_filter == '' and ip_filter == '' and serial_filter == '':
        ctx.my_output.error('Select server by name, ip or serial')
        return None

    if interface_name == '' and interface_mac == '':
        ctx.my_output.error('Define either interface name or interface mac for static IP assignment')
        return None
