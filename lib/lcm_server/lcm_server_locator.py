...
from lib.lcm_server.lcm_server_common import LcmServerCommon


class LcmServerLocator(LcmServerCommon):
    def __init__(self, iaccount, wait=True, dry_run=False, silent=False, verbose=False, debug=False):
        self.dry_run = dry_run
        self.wait = wait
        self.max_start_time = 30
        self.max_complete_time = 60
        LcmServerCommon.__init__(self, iaccount, silent=silent, verbose=verbose, debug=debug)
    ...
    def locator_task(self, servers, admin_state, workflow_name, label):
        ...
        bar_handler.goto(0)
        for server in servers:
            if not self.server_setting_handler.create(server['attributes']):
