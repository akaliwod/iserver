...
def generate_docs_from_templates(results_directory):
    ...
    all_templates_count = get_templates_count(all_templates)

    templates = match_templates_with_results(all_templates, results)
    templates_count = get_templates_count(templates)
    ...
    if templates_count < all_templates_count:
        print('\nFailed Templates:')
        for all_template_group in all_templates:
            for all_template_filename in all_templates[all_template_group]:
                found = False
    ...
    print('\nSummary')
    print('All templates: %s' % (all_templates_count))
    print('Matching templates: %s' % (templates_count))
    if all_templates_count != templates_count:
        return False

    return True


def generate_docs(results_directory, redfish_tests=False):
    ...
    if not generate_docs_from_templates(results_directory):
