...
class ChassisExtraAttributes():
    ...
    def __init__(self, iaccount, settings):
        ...
        self.iocard_handler = equipment_iocard.EquipmentIoCard(iaccount)
        self.ether_host_port_handler = ethernet_host_port.EthernetHostPort(iaccount)
        self.ether_network_port_handler = ethernet_network_port.EthernetNetworkPort(iaccount)
        self.ether_physical_port_handler = ethernet_physical_port.EthernetPhysicalPort(iaccount)
        self.fan_module_handler = equipment_fan_module.EquipmentFanModule(iaccount)
        self.fan_handler = equipment_fan.EquipmentFan(iaccount)
        self.fan_control_handler = equipment_fan_control.EquipmentFanControl(iaccount)
        self.psu_handler = equipment_psu.EquipmentPsu(iaccount)
        self.psu_control_handler = equipment_psu_control.EquipmentPsuControl(iaccount)
        self.power_control_state_handler = power_control_state.PowerControlState(iaccount)
        self.compute_blade_handler = compute_blade.ComputeBlade(iaccount)
        self.adapter_unit_handler = adapter_unit.AdapterUnit(iaccount)
        self.adapter_ext_eth_interface_handler = adapter_ext_eth_interface.AdapterExtEthInterface(iaccount)
        self.cond_alarm_handler = cond_alarm.CondAlarm(iaccount)
        self.tam_advisory_instance_handler = tam_advisory_instance.TamAdvisoryInstance(iaccount)
        self.asset_device_contract_information_handler = asset_device_contract_information.AssetDeviceContractInformation(iaccount)
        self.chassis_profile_handler = chassis_profile.ChassisProfile(iaccount)
        self.network_element_summary_handler = network_element_summary.NetworkElementSummary(iaccount)
